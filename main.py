...
async def get_scout_team(gameweek: int, api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Data not found for gameweek {gameweek}")
    except Exception as e:
...
async def get_player_predictions(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Data not found for gameweek {params.gameweek}")
    except Exception as e:
