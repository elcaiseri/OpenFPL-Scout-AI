...
class FPLScout:
    ...
    def get_player_predictions(self, data_path: str, gameweek: Optional[int] = None) -> pd.DataFrame:
        ...
        gw_matches = fetch_gw_match_data(self.gameweek, self.config["team_name_mapping"])
    ...
    def select_optimal_team(self, predictions: pd.DataFrame) -> pd.DataFrame:
        ...
        team = pd.concat([
            predictions[predictions.element_type == pos].nlargest(count, 'expected_points')
        ...
        logger.info(f"Captain: {team.loc[0, 'web_name']} ({team.loc[0, 'expected_points']:.2f} pts)")
        logger.info(f"Vice-captain: {team.loc[1, 'web_name']} ({team.loc[1, 'expected_points']:.2f} pts)")
...
if __name__ == "__main__":
    ...
    predictions = scout.get_player_predictions("data/external/fpl-data-stats-2.csv", gameweek=1)
